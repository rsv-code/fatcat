...
function JParse (Str) {
    var bo = new BaseObj();
    return bo.mixin(JSON.parse(Str));
}
function JStringify (Obj) {
    return jStringify(Obj, false);
}
$eng = {};
$eng.invokeFunction = function () {
var FunctionName = null, Args = [], i;
    if (arguments.length > 0) {
        ...
        throw ("$eng.invokeFunction(): Expecting at least 1 argument with the function name to invoke.");
    }
    return java.invoke(FunctionName, Args);
};
$eng.eval = function (FileName, ScriptString) {
    if (FileName === undefined) { throw ("Ic9Engine.eval(): File name not set."); }
    if (ScriptString === undefined) { throw ("Ic9Engine.eval(): Script contents not set."); }
    return java.eval(FileName, ScriptString);
};

$eng.bindMethod = function (ObjName, MethName) {
    return function () {
        var args = Array.prototype.slice.call(arguments, 0);
        var cinfo = new BaseObj();
        cinfo.mixin({ objName: ObjName, methName: MethName, args: args });
        return $eng.invokeFunction('callNashornObj', cinfo);
    };
};

$eng.bindObj = function (ObjName) {
    $global[ObjName] = {};
    var meths = $eng.invokeFunction('getFunctNames', ObjName);
    for (var key in meths) {
        var mname = meths[key];
        $global[ObjName][mname] = $eng.bindMethod(ObjName, mname);
    }
};
...
var console = {
    ...
    info: function(message) {
        java.conInfo(message);
    },
    warn: function(message) {
        java.conWarn(message);
    },
    error: function(message) {
        java.conError(message);
    },
    onerror: function(message, url, lineNumber) {
        java.winError(message, url, lineNumber);
    }
};


window.moveTo = function (X, Y) {
    java.moveTo(setDef(X, 0), setDef(Y, 0));
    return this;
}
